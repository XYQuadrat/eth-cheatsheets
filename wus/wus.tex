...
\begin{center}
	\begin{tabularx}{\textwidth}{llXXXX}
		...
		Binomial & \makecell[l] {\( p: \) ErfolgsWK \\ \( n \): Anzahl Versuche} & \( np \) & \( np(1-p) \) & \( \binom{n}{t}p^t(1-p)^{n-t} \) & \( \sum_{k=0}^{t} \binom{n}{k} p^k(1-p)^{n-k} \)  \\
...
\section{Quellen}
Dieses Cheatsheet wurde stark inspiriert vom \href{https://n.ethz.ch/~dcamenisch/summaries}{Cheatsheet von Danny Camenisch}. Ausserdem stammen Teile der Tabellen aus dem Buch ``Formeln, Tabellen und Konzepte''. Die Definitionen sind grösstenteils vom offiziellen Vorlesungsskript (V. Tassion) übernommen.
